...
export const useNotificationStore = create((set, get) => ({
  ...
  createNotification: async (notificationData) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message || error?.message || 'Failed to create notification'
      set({ error: message })
      throw error
    } finally {
      set({ loading: false })
  ...
  broadcastNotification: async (broadcastData) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message || error?.message || 'Failed to broadcast notification'
      set({ error: message })
      throw error
    } finally {
      set({ loading: false })
  ...
  broadcastToRole: async (roleData) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message || error?.message || 'Failed to broadcast to role'
      set({ error: message })
      throw error
    } finally {
      set({ loading: false })
