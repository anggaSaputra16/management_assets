...
export default function OptimizedDecompositionPage() {
  ...
  const fetchDecompositions = useCallback(async (searchParams = {}) => {
    ...
    console.debug('[OptimizedDecomposition] fetchDecompositions start', searchParams)
    ...
    try {
      ...
      if (!abortControllerRef.current.signal.aborted) {
        ...
        console.info('[OptimizedDecomposition] Loaded decompositions:', (response.data || []).length)
      }
      
    } catch (err) {
      if (err.name !== 'AbortError') {
        console.error('[OptimizedDecomposition] fetchDecompositions error', err)
  ...
  const fetchCategories = useCallback(async () => {
    if (categories.length > 0) {
      console.debug('[OptimizedDecomposition] Categories already loaded, skipping...')
      return
    }
    ...
    try {
      ...
      console.debug('[OptimizedDecomposition] Categories loaded:', Array.isArray(catPayload) ? catPayload.length : 0)
    } catch (err) {
      console.error('[OptimizedDecomposition] fetchCategories error', err)
  ...
  const fetchModalAssets = useCallback(async () => {
    ...
    try {
      console.debug('[OptimizedDecomposition] Loading modal assets...')
      ...
      console.info('[OptimizedDecomposition] Modal assets loaded:', (response.data || []).length)
    } catch (err) {
      console.error('[OptimizedDecomposition] fetchModalAssets error', err)
  ...
  useEffect(() => {
    const initializeData = async () => {
      console.debug('[OptimizedDecomposition] Initializing...')
      ...
      try {
        ...
        console.debug('[OptimizedDecomposition] Initialization complete')
        
      } catch (error) {
        console.error('[OptimizedDecomposition] Initialization failed:', error)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.debug('[OptimizedDecomposition] Submitting form...')
      ...
      console.error('[OptimizedDecomposition] Submit error:', err)
  ...
  const handleExecute = async (decomposition) => {
    ...
    try {
      ...
      console.error('[OptimizedDecomposition] Execute error:', err)
  ...
  const handleDelete = async (decomposition) => {
    ...
    try {
      ...
      console.error('[OptimizedDecomposition] Delete error:', err)
